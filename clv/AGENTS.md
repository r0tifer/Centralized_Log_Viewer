...
|--------|----------------|-----------|
| **app.py** | Application shell and orchestrator | - Owns main layout and lifecycle. <br> - Handles global keybindings, routing, and message coordination. <br> - Should not define widget visuals or logic directly. <br> - The tree's Views, Starred and Merged groups are **pure functions of `SessionState`**, reconciled in place and never rebuilt: `_build_tree` creates folders shut, so a rebuild collapses everything the operator had expanded — and every one of those groups now carries click targets, which makes a rebuild a rug pull under the pointer. <br> - A row's click targets are segment metadata keyed `ACTION_META`, valued `<row>-<verb>`; `on_log_tree_action_requested` dispatches on the row type first so an unpaired verb does nothing rather than the wrong thing. <br> - `_star_target` and `_merge_target` answer "what is the cursor pointing at" differently **on purpose** — `_merge_target` takes any ref in the tree at its word and therefore also matches a folder; `_star_target` extends that only to rows the starred group put there. Read both docstrings before unifying them. |
| **cli.py** | The command line | - Owns argv: the three config flags, `doctor`, and the six-command `plugin` group (`list`, `info`, `install`, `verify`, `remove`, `trust`). `main()` here is what both console scripts and `clv/__main__.py` call; `app.main` / `app.run` are shims. <br> - **Bare `clv` launches the viewer and that never changes.** Only a name in `SUBCOMMANDS` diverts, and that tuple is a closed literal — nothing a plugin supplies may reach it, or an installed file could redefine a shell command. <br> - **Imports Textual only on the TUI branch.** `clv doctor` has to run over a pipe and in CI; a test asserts out of process that no UI arrives. <br> - Must be imported *after* `freeze_support()` in `clv/__main__.py`: a frozen build's spawned children arrive as `clv --multiprocessing-fork …`, which this parser would reject as a usage error. <br> - `doctor` reports `PluginRegistry.status()` and never derives rows of its own; `plugin list` uses `discover_user_plugins` and imports nothing. <br> - **No `plugin` subcommand imports plugin code**, asserted by a sentinel plugin parametrised over all six. Handlers take `(args, stream)` and only print; the work is in `plugins/install.py`, so none of it needs argv to be tested. |
...
| **services/ssh_config.py** | Reading OpenSSH's `~/.ssh/config` | - **A reader, never a writer.** That file belongs to OpenSSH and to the operator; CLV parses it when asked and never writes a byte back. <br> - Separate from `services/config.py` because that module owns *CLV's* schema and this one is a guest in somebody else's. Imports `config.py` for `RemoteHost` and the shared `validate_*`; `config.py` never imports it, so the graph stays acyclic. <br> - **Only the alias is imported.** `HostName`/`User`/`Port`/`IdentityFile` are parsed for display and deliberately not persisted: writing `host = 10.0.0.9` would stop `ssh` matching the operator's `Host` block and silently lose `ProxyJump`, per-host keys and `known_hosts`. An imported section is `[ssh:<alias>]` plus `log_dirs`, nothing more. <br> - That also settles `_REFUSED_HOST_KEYS` structurally rather than by a blocklist: `SSHConfigHost` has no field a password or a sudo flag could land in. <br> - Wildcard and `Match` blocks are never candidates, and their keys are never inherited — CLV does not keep a second opinion about a file the `ssh` binary reads for itself. <br> - `ssh_config_path()` expands `~` at **call** time, which is what keeps the suite off the developer's real machine. <br> - Stdlib only; no new dependency. Never raises — a missing, unreadable or self-including file is a note on the scan. |
...
| **plugins/manifest.py** | What a packaged plugin claims | - `clv-plugin.toml` via `tomllib`, per-file SHA-256, the detached signature, and the `.installed/<name>.json` record. <br> - **Not** `plugins/__init__.py`'s `manifest_for()`, which describes a live plugin across the isolation pipe. Same word, unrelated jobs. <br> - Signatures are `ssh-keygen -Y`, always with `-n clv-plugin` — without that namespace a signature made over another file for another purpose replays as a plugin signature. <br> - The signer comes from `find-principals` against the operator's store, never from the manifest: a plugin may not assert who signed it. <br> - **CLV ships no trust root.** `~/.config/clv/plugin-signers` is empty until the operator writes to it. <br> - The manifest is kept byte-for-byte, because a signature covers exact bytes — that is what lets `untrusted` become `verified` when a key is trusted later, with no reinstall. <br> - `annotate()` has two callers (the `P` dialog and `clv doctor`) and reads records only; hashing belongs to `verify`/`doctor`. |
| **plugins/install.py** | Fetching and unpacking hostile input | - The one module here that handles bytes from the internet. A tarball is untrusted whatever its manifest claims about its author. <br> - Extraction is **manual** (`extractfile()` + chunked copy), never `extractall()`: `tarfile`'s filter default differs between the 3.11 floor and 3.14, and `data_filter` does not exist before 3.11.4. <br> - The archive is read as a **stream** (`r|*`): `getmembers()` decompresses everything to build an index, so a bomb would already have cost what the cap prevents. <br> - Members are judged by name *and* kind before a byte is read; symlinks and hardlinks are refused unconditionally, which is what stops a later `mkdir` following one. <br> - The decompressed-byte cap is checked per chunk, not against the header's own claim. <br> - **Nothing reaches the plugin directory until everything passes**, so a refusal leaves it byte-identical — the half of "refused" that is easy to lose. <br> - `https` and `file` only; no cross-host and no scheme-downgrade redirect. |
...
| AdvancedDrawer | `RescanRequested` | Re-run discovery now. |
| AdvancedDrawer | `ScanSSHConfigRequested` | Look in `~/.ssh/config` for machines to import. Carries nothing: the drawer does not read that file, does not know what a host is, and does not write `settings.conf`. |
...
**Check the handler name Textual actually derives.** Its camel-to-snake pass
fuses a run of capitals, so `ScanSSHConfigRequested` wants
`on_advanced_filters_drawer_scan_sshconfig_requested` — not the `_ssh_config_`
spelling that reads naturally. Getting it wrong raises nothing anywhere and
simply leaves the control dead. Read it off `Message.handler_name` rather than
writing it by hand, and pin it in a test.
