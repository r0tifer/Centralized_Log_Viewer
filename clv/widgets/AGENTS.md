...
| **HelpOverlay** | Lists every keybinding | *(dismiss only)* | Modal; sections are built by the app from `BINDINGS`, so it cannot go stale. A modal's **sub-keys** ride in the opening binding's `description` (`"Saved views (then r renames, d deletes)"`) rather than as synthetic rows — one mechanism, and the README key table inherits the same words. `.help-description` is 58 cells at 80 columns and clips silently past that; `test_every_description_fits_the_overlay_at_eighty_columns` is the guard. |
